...
class NoteBloc extends Bloc<NoteEvent, NoteState> {
  ...
  Stream<NoteState> mapEventToState(
    ...
    if (event is GetNoteByKey) {
      yield Loading();
      final model = getNote(GetNoteByKeyParams(key: event.key));
      yield* _eitherFailureOrSuccess(model);
    } else if (event is WriteNoteContent) {
      String content = event.content;
      String key = event.key;
      final model =
          writeContent(WriteContentParams(content: content, key: key));
      yield* _eitherFailureOrSuccess(model);
    } else if (event is WriteNoteTitle) {
      String title = event.title;
      String key = event.key;
      final model = writeTitle(WriteTitleParams(title: title, key: key));
      yield* _eitherFailureOrSuccess(model);
    } else if (event is WriteNoteColor) {
      final Color color = event.color;
      final String key = event.key;
      final model = writeColor(WriteNoteColorParams(color: color, key: key));
      yield* _eitherFailureOrSuccess(model);
      ...
      final model = deleteReminder(DeleteNoteReminderParams(key: event.key));
      yield* _eitherFailureOrSuccess(model);
    } else if (event is DeleteNote) {
      final model = deleteNote(DeleteNoteParams(key: event.key));
      yield* _eitherFailureOrSuccess(model);
    } else if (event is ArchiveNote) {
      final model = archiveNote(ArchiveNoteParams(key: event.key));
      yield* _eitherFailureOrSuccess(model);
